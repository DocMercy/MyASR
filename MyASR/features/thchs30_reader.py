...
class Thchs30Reader:
    ...
    def read_label(self, paths, target_dir):
        chara_target_path = os.path.join(target_dir, 'label', 'chara')
        phone_target_path = os.path.join(target_dir, 'label', 'phone')
        path_check(chara_target_path)
        path_check(phone_target_path)
        ...
        for i in range(len(paths)):
            with open(paths[i], 'r', encoding='utf-8') as a:
                data = a.readlines()
                chara_result.append(data[0].replace('\n', '').replace(' ', ''))
                phone_result.append(data[1].replace('\n', '').split(' '))

            if ((i + 1) % 1000 == 0 and i != 0) or (i + 1) == len(paths):
                with open(os.path.join(chara_target_path, f'label_{count}.pkl'), 'wb') as a:
                    pickle.dump(chara_result, a)
                with open(os.path.join(phone_target_path, f'label_{count}.pkl'), 'wb') as a:
                    pickle.dump(phone_result, a)
                chara_result = []
                phone_result = []
                count += 1
    ...
    def start_reading(self):
        ...
        self.read_label(self.y_train_path, train_save_path)
        self.read_label(self.y_test_path, test_save_path)

        return self.sr


if __name__ == '__main__':
    t = Thchs30Reader(r'D:\Data\data_thchs30', '../temp')
    sr = t.start_reading()
    print(f'音频采样率：{sr}')
